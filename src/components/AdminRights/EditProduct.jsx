...
const EditProduct = ({ id }) => {
    ...
    useEffect(() => {
        // const flattenedArray = products?.flat(1);
        const productsDetails = products?.filter(item => item?._id === id);
        setSingleProduct(productsDetails)
    }, [id])
    console.log('Hello ->', SingleProduct[0]);
    
    return (
        <form
         
            className={` overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] bg-gray-700 bg-opacity-20 md:h-full`}
