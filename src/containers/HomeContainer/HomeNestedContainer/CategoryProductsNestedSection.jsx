...
const CategoryProductsNestedSection = ({ filteredProducts, grid, name }) => {
    ...
    const filteredProductsBestDisc = filteredProducts.filter(product => !bestSellingProductIds.includes(product._id));
    // const filteredProductsBestDisc = discountedProducts.filter(product => !bestSellingProductIds.includes(product._id));
    
    console.log('filteredProductsBestDisc', bestSellingProductIds)
    console.log('filteredProducts', filteredProducts)
    // const filteredProductsBest = discountedProducts.filter(product => bestSellingProductIds.includes(product._id));


    const isMatching = filteredProducts.some(product => bestSellingProductIds.includes(product._id))
    ...
    return (
        <>
            {/* : filteredProductsBestDisc?.length === 0 ? <BestSellingNestedSection sliceProducts={filteredProductsBest} grid={grid} /> */}
            {filteredProducts?.length !== 0 ? (
                <div>
                    <div className={`my-7 grid  gap-7 ${grid}`}>
                        {filteredProductsBestDisc?.map((product) => (
                            <article key={product._id} className="cursor-pointer  flex flex-col justify-between  Parent-Col-Hover relative" >
                                <main>
                                    ...
                                    <section className={`  relative    Parent-product-Image-Hover  `} onClick={() => Navigate(`/viewsingleproduct/${product?._id}/${false}/${false}/newProduct`)}>
                                        ...
                                        <span className=' md:leading-5 mb-3 mt-4   leading-5 line-clamp-2   md:text-lg font-bold   hover:cursor-pointer hover:underline  Parent-product-text-Hover  capitalize  '>{product.name}</span>
                        ...
                        {isMatching===true && <BestSellingNestedSection categoryFilterdProduct={filteredProducts} /> }
            ...
            {/* <BestSellingNestedSection categoryFilterdProduct={filteredProductsBest}  grid={grid} /> */}
        </>
    )
}
